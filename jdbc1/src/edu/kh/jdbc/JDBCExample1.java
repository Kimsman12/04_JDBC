...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// ==DBerver 에서 사용하는 DB연결과 같은 역할의 객체
		// (DB서버 주소, 포트번호, DB이름, 계정명, 비밀번호)
		...
		String dbName = ":XE"; // DBMS 이름(XE == eXpressEdition)
		...
		// -> DB 연결 정보가 잘못되거나 객체 생성에 문제가 생기면 예외발생
		...
		// -> sql이 SELECT 문일때 결과로 java.sql.ResultSet 반환
		
		// 2) int Statement.excuteUpdate(sql)
		// -> DML(INSERT, UPDATE, DELETE)
		// 결과로 int형 반환 (삽입, 수정, 삭제된 행의 개수)
		
		rs = stmt.executeQuery(sql);
		
		/* 6. 조회 결과가 담겨있는 ResultSet을
		 *    커서(Cursor)를 이용해 1행씩 접근해 각 행에 작성된 컬럼값 얻어오기
		 *    
		 * */
		...
		while (rs.next()) {
			...
			// -> 수행하지 않으면 .DB 와 연결된 Connection 이 그대로 남아있어서
			// 다른 클라이언트가 추가적으로 연결되지 못하는 문제가 발생할 수 있다
			try {
				// 만들어진 역순으로 close 수행을 권장
				if( rs != null ) rs.close();
